...
welcome_screen() {
  whiptail --title "Self Hosted Maps" --msgbox "This installer will set up a native offline-capable map stack on Debian.\n\nIt will update the system, install dependencies, configure services, build tiles, and register a cron job." 14 72
}
...
choose_region_url() {
  ...
  case "$choice" in
    ...
    custom)
      whiptail --title "Custom PBF URL" --inputbox "Enter the full URL to a .osm.pbf file:" 10 80 3>&1 1>&2 2>&3
      ;;
  esac
}
